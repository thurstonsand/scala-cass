...
abstract class ScalaCassUnitTests
    extends CassandraWithTableTester(
      "testDB",
      "testTable",
      ScalaCassUnitTestsVersionSpecific.extraHeaders ::: List(
        "str varchar",
        "str2 ascii",
        "b blob",
        "d decimal",
        ...
        "s set<double>",
        "id uuid",
        "sblob set<blob>, tup tuple<int, varchar>",
        "nest map<varchar, frozen<set<int>>>"
      ),
      List("str")
    )
    with OptionValues {
  def testType[GoodType: CassFormatDecoder, BadType: CassFormatDecoder](
    ...
    val args = {
      val converted =
        goodCF.encode(v).fold(throw _, identity).asInstanceOf[AnyRef]
      if (k == "str") Seq((k, converted))
      else Seq((k, converted), ("str", "asdf"))
    }
    insert(args)
    val res = getOne
    k match {
      case "sblob" =>
        ...
        res
          .as[GoodType](k)
          .asInstanceOf[Iterable[Array[Byte]]]
          .head should contain theSameElementsInOrderAs known
        res
          .getAs[GoodType](k)
          .map(_.asInstanceOf[Iterable[Array[Byte]]].head)
          .value should contain theSameElementsInOrderAs known

        res
          .getOrElse(k, default)
          .asInstanceOf[Iterable[Array[Byte]]]
          .head should contain theSameElementsInOrderAs known
        an[IllegalArgumentException] should be thrownBy res
          .getOrElse(s"not$k", default)
          .asInstanceOf[Iterable[Array[Byte]]]
          .head

        res
          .attemptAs[GoodType](k)
          .toOption
          .map(_.asInstanceOf[Iterable[Array[Byte]]].head)
          .value should contain theSameElementsInOrderAs known
      case _ =>
        ...
        an[IllegalArgumentException] should be thrownBy res.getOrElse(
          s"not$k",
          default
        )
        res.attemptAs[GoodType](k).getOrElse(None) shouldBe v
    }
    ...
    an[IllegalArgumentException] should be thrownBy res.getAs[GoodType](
      s"not$k"
    )
    ...
    if (testCC) {
      ...
      k match {
        case "b" =>
          ss.selectOneStar(tname, ScalaSession.NoQuery())
            .execute()
            .toOption
            .flatten
            .flatMap(_.getAs[TestCC])
            .map(_.refField.asInstanceOf[Array[Byte]])
            .value should contain theSameElementsInOrderAs t1.refField
        ...
        case "sblob" =>
          ss.selectOneStar(tname, ScalaSession.NoQuery())
            ...
            .flatten
            .flatMap(_.getAs[TestCC])
            .flatMap(_.refField.asInstanceOf[Set[Array[Byte]]].headOption)
            .value should contain theSameElementsInOrderAs t1.refField
            .asInstanceOf[Set[Array[Byte]]]
            .head
        case _ =>
          ss.selectOneStar(tname, q1)
            .execute()
            .toOption
            .flatten
            .flatMap(_.getAs[TestCC])
...
class ScalaCassUnitTestsAll
  ...
  "ints" should "be extracted correctly" in testType[Int, String](
    "i",
    1234,
    9876
  )
  "bigints" should "be extracted correctly" in testType[Long, String](
    "bi",
    1234,
    9876
  )
  "boolean" should "be extracted correctly" in testType[Boolean, Int](
    "bool",
    true,
    false
  )
  "double" should "be extracted correctly" in testType[Double, String](
    "dub",
    123.4,
    987.6
  )
  "list" should "be extracted correctly (wrong basic)" in testType[List[String], String](
    "l",
    List("as", "df"),
    List("fd", "sa")
  )
  "list" should "be extracted correctly (wrong type param)" in testType[List[
    String
  ], List[Int]]("l", List("as", "df"), List("fd", "sa"))
  "map" should "be extracted correctly (wrong basic)" in testType[Map[
    String,
    Long
  ], String]("m", Map("asdf" -> 10L), Map("fdsa" -> -10L))
  "map" should "be extracted correctly (wrong 1st type param)" in testType[Map[
    String,
    Long
  ], Map[Long, Long]]("m", Map("asdf" -> 10L), Map("fdsa" -> -10L))
  "map" should "be extracted correctly (wrong 2nd type param)" in testType[Map[
    String,
    Long
  ], Map[String, Int]]("m", Map("asdf" -> 10L), Map("fdsa" -> -10L))
  ...
  "map<varchar, set<int>>" should "be extracted correctly" in testType[Map[
    String,
    Set[Int]
  ], Map[String, Int]](
    "nest",
  ...
  "set" should "be extracted correctly (wrong basic)" in testType[Set[Double], String](
    "s",
    Set(123.4),
    Set(987.6)
  )
  "set" should "be extracted correctly (wrong type param)" in testType[Set[
    Double
  ], Set[String]]("s", Set(123.4), Set(987.6))
  "uuid" should "be extracted correctly" in testType[java.util.UUID, String](
    "id",
    java.util.UUID.randomUUID,
    java.util.UUID.randomUUID
  )
  "ascii" should "be extracted correctly" in testType[String, Int](
    "str2",
    "asdf",
    "fdsa"
  )
  "blob" should "be extracted correctly (wrong basic)" in testType[Array[Byte], String](
    "b",
    "asdf".getBytes,
    "fdsa".getBytes
  )
  "inet" should "be extracted correctly" in testType[
  ...
  "decimal" should "be extracted correctly" in testType[BigDecimal, Double](
    "d",
    BigDecimal(3.0),
    BigDecimal(2.0)
  )
  "varint" should "be extracted correctly" in testType[BigInt, Long]("vi", 3, 2)
  "float" should "be extracted correctly" in testType[Float, Double](
    "f",
    123.4f,
    987.6f
  )
  "set<blob>" should "be extracted correctly" in testType[Set[Array[Byte]], Set[
    Double
  ]]("sblob", Set("asdf".getBytes), Set("fdsa".getBytes))
  "tup<int, varchar>" should "be extracted correctly (wrong basic)" in testType[
    (Int, String),
    String
  ]("tup", (4, "fdsa"), (5, "asas"))
  "tup<int, varchar>" should "be extracted correctly (wrong 1st type)" in testType[
    (Int, String),
    (String, String)
  ]("tup", (4, "fdsa"), (5, "qqwer"))
  "tup<int, varchar>" should "be extracted correctly (wrong arity)" in {
    ...
    val args = Seq(
      (
        "tup",
        implicitly[CassFormatEncoder[(Int, String)]]
          .encode(goodValue)
    ...
    an[IllegalArgumentException] should be thrownBy res.as[(Int, String)](
      "nottup"
    )
    an[InvalidTypeException] should be thrownBy res.as[(Int, String, String)](
      "tup"
    )
  ...
  "counter" should "be extracted correctly" in {
    ...
    client.session.execute(
      s"UPDATE $dbName.$counterTable SET $k = $k + ? WHERE $pKey='asdf'",
      Long.box(1L)
    )

    val res =
      client.session.execute(s"SELECT * FROM $dbName.$counterTable").one()
