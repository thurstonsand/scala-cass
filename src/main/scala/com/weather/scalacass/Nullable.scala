...
object Nullable {
  ...
  implicit def nullable2option[A](nullable: Nullable[A]): Option[A] =
    nullable.toOption
  ...
  implicit def encoder[A](
    ...
    def encode(f: Nullable[A]): Result[Nullable[underlying.From]] = f match {
      case Is(x)     => underlying.encode(x).map(Is.apply)
      case IsNotNull => Right(IsNotNull)
      case IsNull    => Right(IsNull)
    }

    override def withQuery(instance: Nullable[A], name: String): String =
      instance match {
  ...
  implicit def decoder[A](
    ...
    override def tupleDecode(tup: TupleValue, pos: Int): Result[Nullable[A]] =
