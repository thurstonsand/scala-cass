  ...
  private[scalacass] def tupleDecode(tup: TupleValue, pos: Int): Result[T] =
    Try[Result[T]](
      if (tup.isNull(pos))
        Left(
          new ValueNotDefinedException(
            s"""position $pos was not defined in tuple $tup"""
          )
        )
  ...
  implicit def tupleFormat[TUP <: Product](
    ...
    override def decode(r: Row, name: String): Result[TUP] =
      super.decode(r, name) match {
        case Left(f: java.lang.ArrayIndexOutOfBoundsException) =>
          Left(new InvalidTypeException("tuple of wrong arity", f))
        case other => other
      }
    override def tupleDecode(tup: TupleValue, pos: Int): Result[TUP] =
      super.tupleDecode(tup, pos) match {
        case Left(f: java.lang.ArrayIndexOutOfBoundsException) =>
          Left(new InvalidTypeException("tuple of wrong arity", f))
        case other => other
      }
...
object CassFormatDecoder extends CassFormatDecoderVersionSpecific {
  ...
  def apply[T](implicit decoder: CassFormatDecoder[T]): CassFormatDecoder[T] =
    decoder
  ...
  implicit val stringFormat: CassFormatDecoder[String] =
    sameTypeCassFormatDecoder(
      TypeToken.of(classOf[String]),
      _ getString _,
      _ getString _
    )
  implicit val uuidFormat: CassFormatDecoder[java.util.UUID] =
    sameTypeCassFormatDecoder(
      TypeToken.of(classOf[java.util.UUID]),
      _ getUUID _,
      _ getUUID _
    )
  implicit val iNetFormat: CassFormatDecoder[java.net.InetAddress] =
    sameTypeCassFormatDecoder[java.net.InetAddress](
      TypeToken.of(classOf[java.net.InetAddress]),
      _ getInet _,
      _ getInet _
    )

  implicit val intFormat: CassFormatDecoder[Int] =
    safeConvertCassFormatDecoder[Int, java.lang.Integer](
  ...
  implicit val longFormat: CassFormatDecoder[Long] =
    safeConvertCassFormatDecoder[Long, java.lang.Long](
      TypeToken.of(classOf[java.lang.Long]),
      Long.unbox,
      _ getLong _,
      _ getLong _
    )
  implicit val booleanFormat: CassFormatDecoder[Boolean] =
    safeConvertCassFormatDecoder[Boolean, java.lang.Boolean](
      TypeToken.of(classOf[java.lang.Boolean]),
      Boolean.unbox,
      _ getBool _,
      _ getBool _
    )
  implicit val doubleFormat: CassFormatDecoder[Double] =
    safeConvertCassFormatDecoder[Double, java.lang.Double](
      TypeToken.of(classOf[java.lang.Double]),
      Double.unbox,
      _ getDouble _,
      _ getDouble _
    )
  implicit val floatFormat: CassFormatDecoder[Float] =
    safeConvertCassFormatDecoder[Float, java.lang.Float](
      TypeToken.of(classOf[java.lang.Float]),
      Float.unbox,
      _ getFloat _,
      _ getFloat _
    )
  implicit val bigIntegerFormat: CassFormatDecoder[BigInt] =
    safeConvertCassFormatDecoder[BigInt, java.math.BigInteger](
      TypeToken.of(classOf[java.math.BigInteger]),
      BigInt.javaBigInteger2bigInt,
      _ getVarint _,
      _ getVarint _
    )
  implicit val bigDecimalFormat: CassFormatDecoder[BigDecimal] =
    safeConvertCassFormatDecoder[BigDecimal, java.math.BigDecimal](
      TypeToken.of(classOf[java.math.BigDecimal]),
  ...
  def listOf[T](eltType: TypeToken[T]): TypeToken[java.util.List[T]] =
    new TypeToken[java.util.List[T]]() {}
  ...
  implicit def setFormat[T](
    ...
    val typeToken: TypeToken[java.util.Set[underlying.From]] = setOf(
      underlying.typeToken
    )
    def f2t(f: From): Result[Set[T]] = {
      val acc = Set.newBuilder[T]
      @scala.annotation.tailrec
      def process(it: java.util.Iterator[underlying.From]): Result[Set[T]] =
        if (!it.hasNext) Right(acc.result)
        else {
          underlying.f2t(it.next()) match {
            case Right(h) =>
      ...
      def f2t(f: From): Result[Map[K, V]] = {
        ...
        def process(
          ...
          if (!it.hasNext) Right(acc.result)
            ...
            val n = it.next()
            (for {
              k <- underlyingK.f2t(n.getKey)
              v <- underlyingV.f2t(n.getValue)
            } yield (k, v)) match {
              case Right((k, v)) =>
                acc += k -> v; process(it)
              case Left(exc) => Left(exc)
  ...
  implicit val blobFormat: CassFormatDecoder[Array[Byte]] =
    new CassFormatDecoder[Array[Byte]] {
        ...
        Try[Result[Array[Byte]]](
          if (tup.isNull(pos))
            ...
            if (cassName != DataType.Name.BLOB)
              Left(
                new InvalidTypeException(
                  s"position $pos in tuple $tup is not a blob"
                )
              )
            else f2t(tupleExtract(tup, pos))
          }
  ...
  implicit def optionFormat[A](
    ...
    override def tupleDecode(tup: TupleValue, pos: Int): Result[Option[A]] =
      super.tupleDecode(tup, pos) match {
        case Left(Recoverable(_)) => Right(None)
        case other                => other
      }
  ...
  implicit def eitherFormat[A](
    ...
    def tupleExtract(tup: TupleValue, pos: Int) =
      underlying.tupleExtract(tup, pos)
    override def tupleDecode(tup: TupleValue, pos: Int) =
      super.tupleDecode(tup, pos) match {
        case Left(l) => Right(Left(l))
        case same    => same
      }
