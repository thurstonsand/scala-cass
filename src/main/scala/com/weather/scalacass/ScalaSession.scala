...
final case class UpdateBatch[T, S](table: String, item: T, query: T)(implicit val tEncoder: CCCassFormatEncoder[T],
                                                                     val sEncoder: CCCassFormatEncoder[S])
    extends Batch
...
class ScalaSession(val keyspace: String)(implicit val session: Session) {
  ...
  @SuppressWarnings(
    Array("org.brianmckenna.wartremover.warts.Any",
          "org.brianmckenna.wartremover.warts.AsInstanceOf",
          "org.brianmckenna.wartremover.warts.IsInstanceOf"))
  private[this] def clean[T](toClean: List[(String, AnyRef)]): (List[String], List[AnyRef]) =
    toClean
      .filter(_._2 match {
        case None => false
        case Some(_) => true
  ...
  def createTable[T: CCCassFormatEncoder](name: String,
                                          numPartitionKeys: Int,
                                          numClusteringKeys: Int,
                                          tableProperties: String = ""): ResultSet = {
    ...
    if (numPartitionKeys + numClusteringKeys > allColumns.length)
      throw new WrongPrimaryKeySizeException(s"too many partition+clustering keys for table $name")
    ...
    session.execute(
      s"CREATE TABLE $keyspace.$name (${allColumns.map(nt => s"${nt._1} ${nt._2}").mkString(", ")}, PRIMARY KEY $fullKey)" + withClause)
  }
  ...
  private[this] def prepareInsert[T: CCCassFormatEncoder](table: String, insertable: T): BoundStatement = {
    ...
    val prepared = queryCache.getP(
      ...
      session.prepare(s"INSERT INTO $keyspace.$table ${strArgs
        .mkString("(", ",", ")")} VALUES ${List.fill(anyrefArgs.length)("?").mkString("(", ",", ")")}")
    )
    prepared.bind(anyrefArgs: _*)
  }

  def insert[T: CCCassFormatEncoder](table: String, insertable: T): ResultSet =
    session.execute(prepareInsert(table, insertable))
  def insertAsync[T: CCCassFormatEncoder](table: String, insertable: T): Future[ResultSet] =
    session.executeAsync(prepareInsert(table, insertable))
  ...
  private[this] def prepareUpdate[T: CCCassFormatEncoder, S: CCCassFormatEncoder](table: String,
                                                                                  updateable: T,
                                                                                  query: S): BoundStatement = {
    ...
    val prepared = queryCache.getP(
      ...
      session.prepare(
        s"UPDATE $keyspace.$table SET ${updateStrArgs.map(_ + "=?").mkString(",")} WHERE ${queryStrArgs.map(_ + "=?").mkString(" AND ")}")
    )
  ...
  def batch(batches: Seq[Batch], batchType: BatchStatement.Type = BatchStatement.Type.LOGGED): ResultSet =
    session.execute(prepareBatch(batches, batchType))
  def batchAsync(batches: Seq[Batch], batchType: BatchStatement.Type = BatchStatement.Type.LOGGED): Future[ResultSet] =
    session.executeAsync(prepareBatch(batches, batchType))

  private[this] def prepareSelect[Sub: CCCassFormatEncoder, Query: CCCassFormatEncoder](table: String,
                                                                                        selectable: Query,
    ...
    val prepared = queryCache.getP(sStrArgs.toSet ++ qStrArgs.toSet + table + "SELECT", {
      ...
      session.prepare(
        s"SELECT ${sStrArgs.mkString(", ")} FROM $keyspace.$table" + whereClauseStr + limitStr + filteringStr)
    })
    prepared.bind(qAnyRefArgs: _*)
  }

  def select[T: CCCassFormatEncoder](table: String,
                                     selectable: T,
                                     allowFiltering: Boolean = false,
                                     limit: Long = 0): Iterator[Row] =
    session.execute(prepareSelect[Star, T](table, selectable, allowFiltering, limit)).iterator.asScala
  def selectAsync[T: CCCassFormatEncoder](table: String,
                                          selectable: T,
                                          allowFiltering: Boolean = false,
                                          limit: Long = 0): Future[Iterator[Row]] =
  ...
  def selectOneAsync[T: CCCassFormatEncoder](table: String,
                                             selectable: T,
                                             allowFiltering: Boolean = false): Future[Option[Row]] =
    session.executeAsync(prepareSelect[Star, T](table, selectable, allowFiltering, 0)).map(rs => Option(rs.one()))

  def selectColumns[Sub: CCCassFormatEncoder, Query: CCCassFormatEncoder](table: String,
                                                                          selectable: Query,
                                                                          allowFiltering: Boolean = false,
                                                                          limit: Long = 0): Iterator[Row] =
    session.execute(prepareSelect[Sub, Query](table, selectable, allowFiltering, limit)).iterator.asScala
  def selectColumnsAsync[Sub: CCCassFormatEncoder, Query: CCCassFormatEncoder](table: String,
                                                                               selectable: Query,
                                                                               allowFiltering: Boolean = false,
                                                                               limit: Long = 0): Future[Iterator[Row]] =
    session.executeAsync(prepareSelect[Sub, Query](table, selectable, allowFiltering, limit)).map(_.iterator.asScala)
  def selectColumnsOne[Sub: CCCassFormatEncoder, Query: CCCassFormatEncoder](
      table: String,
      selectable: Query,
      allowFiltering: Boolean = false): Option[Row] =
    Option(session.execute(prepareSelect[Sub, Query](table, selectable, allowFiltering, 0)).one())
  def selectColumnsOneAsync[Sub: CCCassFormatEncoder, Query: CCCassFormatEncoder](
      table: String,
      selectable: Query,
      allowFiltering: Boolean = false): Future[Option[Row]] =
